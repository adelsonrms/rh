...
function RegistrarEventosAjax() {
    ...
    $(document).ajaxSend(function (event, req, options) {
        ...
        LogEvento('Global.' + event.type, ['Request enviado para o servidor', options.type + ' ' + options.url, options]);
        if (bSend == 0) { bSend = 1 }
    })
    $(document).ajaxSuccess(function (event, req, options, options, data) {
    ...
    $(document).ajaxComplete(function (event, req, options) {
        LogEvento('Global.' + event.type, ['Um Request foi finalizado', , options.type + ' ' + options.url]);
    })
...
function jqajax(metodo, url, parametros, callbacks) {
    ...
    var jqxhr = $.ajax(
        url,
        {
            ...
            data: Params, //Parametros que serão passados para a função (URL)
