   ...
   public class FuncionarioViewModel
    {
        ...
        [RegularExpression(@"^(?:(?:\+|00)?(55)\s?)?(?:\(?([1-9][0-9])\)?\s?)?(?:((?:9\d|[2-9])\d{3})\-?(\d{4}))$", 
            ErrorMessage = "O Numero do Telefone é Inválido")]
        public  string Telefone { get; set; }

        [Phone]
        [DataType(DataType.PhoneNumber)]
        [RegularExpression(@"^(?:(?:\+|00)?(55)\s?)?(?:\(?([1-9][0-9])\)?\s?)?(?:((?:9\d|[2-9])\d{3})\-?(\d{4}))$", 
            ErrorMessage = "O Numero do Celular é Inválido")]
        public  string Celular { get; set; }

        [DisplayName("Email/Login")]
        [Required(ErrorMessage = "Informe o email do funcionario")]
        [EmailAddress]
        [RegularExpression(@".+\@tecnun\.com\.br$", 
            ErrorMessage = "Somente endereços tecnun.com.br são validos nesse campo")]
